...
df <- read.csv("data/created/tables/landscape_genetics_data.csv",
                 sep = ",", header = T)
...
for(i in 1:nrow(df)){
  ...
  for(j in 1:length(names(rst))){
    ...
    df[i,paste0(names(rst_temp))] <- rst_mean
...
write.table(df, "data/created/tables/landscape_genetics_data_with_env.csv",
            sep = ",", row.names = F)

# Investigate
ggplot(df, aes(x = Dist_release_km,
               y = HI_nDNA, color = Grass)) +
